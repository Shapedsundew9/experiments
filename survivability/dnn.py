...
def load_data():
    ...
    hashes = {
        k: [blake2b(x, digest_size=8).hexdigest() for x in data[k]]
    ...
    if not exists(_DATA_HASHES_FILE_BASE):
        ...
        print(
            "Validating data reproducibility with image data hash file: "
            + _DATA_HASHES_FILE_BASE
        )
        for data, valid in tqdm(
            zip(hashes.items(), valid_hashes.items()), desc="Data sets"
        ):
            if not all([d == v for d, v in zip(data[1], valid[1])]):
                print(
                    "Loaded "
                    + data[0]
                    + " data set not consistent with image data hash file contents."
                )
                exit(1)
    return data
...
def _evaluate(model, data):
    ...
    results = model.evaluate(
        data["x_test"], data["y_test"], batch_size=64, verbose=0, return_dict=True
    )
...
def _predict(model, data):
    ...
    data (dict): See load_data() return value for structure.
    """
    return model.predict(data["x_test"], batch_size=64, verbose=0)
...
def align_weights(model, ref_model):
    ...
    for i, (layer, ref_layer) in enumerate(pairs):
        if not i & 1:
            ...
            # ...
            new_order = linear_sum_assignment(
                norm(layer[:, newaxis] - ref_layer, axis=2)
            )[1]
    ...
    for layer, new_layer in zip(layers, new_layers):
        ...
        if len(layer.shape) == 2:
            old_hashes = {
                hash(sort(row).data.tobytes())
                for row in layer.reshape(layer.shape[::-1])
            }
            new_hashes = {
                hash(sort(row).data.tobytes())
                for row in new_layer.reshape(new_layer.shape[::-1])
            }
...
def experiment_1(model):
        ...
        for i in trange(_POPULATION, desc="Exp 1: Initial separation"):
            for j in range(i + 1, _POPULATION):
                start_distances.append(
                    {"model_i": i, "model_j": j, "distance": norm(data[i] - data[j])}
                )
    ...
    # TODO: Add to plot
    plot_histogram(
        [d["distance"] for d in start_distances],
        "Separations of initial starting positions.",
        _EXP1_SD_BASE,
    )
    ...
    plot_histogram(
        end_distances, "Separations of found minima positions.", _EXP1_ED_BASE
    )
    plot_histogram(
        travel_distances,
        "Distance travelled from start position to minima position.",
        _EXP1_TD_BASE,
    )
    ...
    if not exists(f"{_EXP1_SA_BASE}.json.gz"):
        ...
        for i in trange(_POPULATION, desc="Exp 1: Initial accuracy"):
            start_accuracy.append(
                evaluate(load_weights(model, f"{_EXP1_SW_BASE}_{i:06d}"))
            )
    ...
    if not exists(f"{_EXP1_EA_BASE}.json.gz"):
        ...
        for i in trange(_POPULATION, desc="Exp 1: Final accuracy"):
            end_accuracy.append(
                evaluate(load_weights(model, f"{_EXP1_EW_BASE}_{i:06d}"))
            )
...
def experiment_2(model):
    ...
    if not exists(f"{_EXP2_EO_BASE}.npy"):
        data = empty(
            (
                _POPULATION,
                _VALIDATION_SAMPLE_NUM,
                model.get_layer("predictions").output_shape[1],
            )
        )
                ...
                output_distances.append(
                    {
                        "model_i": i,
                        "model_j": j,
                        "distance": norm(outputs[i] - outputs[j]),
                    }
                )
    ...
    plot_histogram(
        [d["distance"] for d in output_distances],
        "Separations of test set predictions.",
        _EXP2_OS_BASE,
    )
...
def experiment_3(model):
    ...
    weights_i = flatten_weights(
        load_weights(model, f'{_EXP1_SW_BASE}_{closest_pair["model_i"]:06d}')
    )
