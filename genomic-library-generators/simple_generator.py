...
from egp_types.xgc_validator import LGC_json_load_entry_validator, LGC_entry_validator, LGC_json_dump_entry_validator
...
for generation in trange(_NUM_GENERATIONS):
    ...
    ngc.update({
        ...
        'code_depth': max((gcs[ngc['gca']]['code_depth'], gcs[ngc['gcb']]['code_depth'])) + 1,
        'codon_depth': gcs[ngc['gca']]['codon_depth'] + gcs[ngc['gcb']]['codon_depth'],
        'num_codes': gcs[ngc['gca']]['num_codes'] + gcs[ngc['gcb']]['num_codes'] + 1,
        'num_codons': gcs[ngc['gca']]['num_codons'] + gcs[ngc['gcb']]['num_codons']
    })
    gcs[ngc['gca']]['reference_count'] += 1
    gcs[ngc['gcb']]['reference_count'] += 1
    assert LGC_entry_validator.validate(ngc), f'\n{LGC_entry_validator.error_str()}\n------------\n{pformat(ngc, indent=4, width=120)}'
    ngc = LGC_entry_validator.normalized(ngc)
    gcs[ngc['signature']] = ngc

with open('gcs.json', 'w', encoding='utf-8') as fptr:
    dump([LGC_json_dump_entry_validator.normalized(ngc) for ngc in gcs.values()], fptr, indent=4, sort_keys=True)
